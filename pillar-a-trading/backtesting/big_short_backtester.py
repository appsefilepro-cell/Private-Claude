...
class BigShortBacktester:
    ...
    def backtest_historical_data(self) -> Dict[str, Any]:
        ...
        avg_profit = (total_profit / correct_predictions) if correct_predictions > 0 else 0
        ...
        # Save results
        output_file = self.results_path / f"big_short_backtest_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def run_monte_carlo_simulation(self, num_simulations: int = 10000) -> Dict[str, Any]:
        """
...
"""
        ...
        top_trades = sorted(results['trades'], key=lambda x: x['profit_pct'], reverse=True)[:5]
