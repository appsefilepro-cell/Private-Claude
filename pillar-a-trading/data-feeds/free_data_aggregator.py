...
class FreeDataAggregator:
    ...
    def _get_alpha_vantage_data(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            if key == 'demo':
                logger.warning("Using Alpha Vantage demo key - get free key at alphavantage.co")
    ...
    def _get_news_sentiment(self, symbol: str) -> Optional[Dict]:
        ...
        try:
            ...
            positive_words = ['rise', 'gain', 'bull', 'surge', 'rally', 'profit', 'growth', 'up', 'high', 'strong']
            negative_words = ['fall', 'loss', 'bear', 'drop', 'crash', 'decline', 'down', 'low', 'weak']
    ...
    def generate_trading_signal(self, symbol: str) -> Dict[str, Any]:
        ...
        if yahoo:
            ...
            if change_percent > 2:  # Strong upward movement
                confidence_score += 12
                signal['reasons'].append(f'Strong price momentum (+{change_percent:.2f}%)')
                signal['action'] = 'BUY'
            elif change_percent < -2:  # Strong downward movement
                confidence_score += 12
                signal['reasons'].append(f'Strong downward momentum ({change_percent:.2f}%)')
                signal['action'] = 'SELL'
        ...
        signal['metrics'] = {
            ...
            'data_quality': len(data['sources_used']) / 10,  # Quality based on number of sources
            'target_met': signal['confidence'] >= 0.91
        }

        logger.info(f"✅ Signal: {signal['action']} @ {signal['confidence']:.2%} confidence")
        logger.info(f"   Sources used: {len(data['sources_used'])}")
        logger.info(f"   Target accuracy: {'ACHIEVED' if signal['metrics']['target_met'] else 'IN PROGRESS'}")
