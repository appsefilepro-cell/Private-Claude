...
class BigShortStrategy:
    ...
    def analyze_for_short(self, symbol: str, data: Dict) -> Dict:
        ...
        try:
            pe_ratio = (
                float(pe_ratio) if pe_ratio not in ["N/A", None] else 999
            )  # Assume very high if N/A
        ...
        try:
            debt_equity = (
                float(debt_equity)
                if debt_equity not in ["N/A", None, "Unknown (fraud)"]
                else 0
            )
        except (ValueError, TypeError):
            debt_equity = 0
        ...
        if pe_ratio > self.criteria["pe_ratio_max"]:
            ...
            signal["reasons"].append(
                f"Extremely high P/E: {pe_ratio if pe_ratio < 999 else 'N/A (no earnings)'} (overvalued)"
            )
        ...
        if earnings_quality == "suspicious" or data.get(
    ...
    def calculate_position_size(
        self, account_balance: float, confidence: float, risk_per_trade: float = 0.02
    ) -> Dict:
        """
...
def main():
    ...
    position_info = strategy.calculate_position_size(
        account_balance, signal["confidence"]
    )
