...
def main():
    ...
    content = [
        {'type': 'title', 'text': 'Agent X5.0 Report'},
        {'type': 'text', 'text': 'This is an automated report generated by Agent X5.0 system.'},
        {'type': 'text', 'text': f'Generated on: {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}'},
    ]
    
    result = processor.generate_pdf(
        output_path='/tmp/agent_x5_report.pdf',
        content=content,
        title='Agent X5.0 Report'
    )
